...
if "`1'" == "" | "`2'" == "" | "`3'" == "" {
    display as error "Usage: stata -b do experiment5.do N model rho [output_dir]"
...
// ========================================================================
// SIMULATION PARAMETERS - EXPERIMENT 5: NON-STATIONARY TIME-VARYING ERRORS
// ========================================================================
...
// Selection equation parameters (baseline)
local a_param 1.8            // Set so P(d_it* > 0) = 0.85 (15% selection)
...
// Outcome equation parameters (baseline structure, modifications below)
local sigma_alpha0 1         // Standard deviation of alpha_i^0 ~ N(0,1)
local sigma_eps0 1           // Standard deviation of epsilon_it^0 ~ N(0,1)
local theta_param 0.5        // Base correlation parameter (will be modified)
local vartheta_param 0.5     // Base correlation parameter (will be modified)
...
display as text ""
display as text "========================================================================="
display as text "EXPERIMENT 5: NON-STATIONARY TIME-VARYING ERRORS - ENDOGENOUS SELECTION"
display as text "========================================================================="
display as text "Parameters: N=`N', Model=`model', rho=`rho', Replications=`reps'"
display as text "EXPERIMENT: Non-stationary time-varying error components"
display as text "- Time-varying variance: εᵢₜ or uᵢₜ multiplied by Bernoulli(1,2)"
display as text "- Time-varying correlation: ϑ multiplied by {0.5, 1, 2}"
display as text "Selection: 15% attrition, ENDOGENOUS with time-varying parameters"
display as text "========================================================================="
display as text ""
...
forvalues rep = 1/`reps' {
    ...
    // ====================================================================
    // DATA GENERATION PROCESS (Non-stationary time-varying errors)
    // ====================================================================
    ...
    quietly {
        ...
        // Generate base time-varying components
        gen double eps_i0 = rnormal(0, `sigma_eps0')
        gen double u_it_base = rnormal(0, `sigma_u')
        gen double z_it = rnormal(0, `sigma_z')
        
        // EXPERIMENT 5 MODIFICATIONS:
        
        // 1. Time-varying variance: multiply εᵢₜ or uᵢₜ by Bernoulli(1,2)
        gen double variance_multiplier_eps = (runiform() < 0.5) + 1  // Takes values 1 or 2
        gen double variance_multiplier_u = (runiform() < 0.5) + 1    // Takes values 1 or 2
        
        // Apply time-varying variance to both error components
        gen double eps_i0_scaled = eps_i0 * variance_multiplier_eps
        gen double u_it = u_it_base * variance_multiplier_u
        
        // 2. Time-varying correlation: multiply ϑ by {0.5, 1, 2}
        gen double correlation_multiplier = 0.5 if runiform() < 0.333
        replace correlation_multiplier = 1 if missing(correlation_multiplier) & runiform() < 0.5
        replace correlation_multiplier = 2 if missing(correlation_multiplier)
        
        // Apply time-varying correlation parameters
        gen double theta_it = `theta_param' * correlation_multiplier
        gen double vartheta_it = `vartheta_param' * correlation_multiplier
        
        // Generate correlated error components with time-varying parameters
        gen double alpha_i = alpha_i0 + theta_it * eta_i
        gen double eps_it = eps_i0_scaled + vartheta_it * u_it
...
display as text ""
display as text "========================================================================="
display as text "EXPERIMENT 5 SIMULATION COMPLETED"
display as text "========================================================================="
...
matrix Results[1,9] = 5

// Display final results table
display as text ""
display as text "========================================================================="
display as text "EXPERIMENT 5 FINAL RESULTS - NON-STATIONARY TIME-VARYING ERRORS"
display as text "Model `model', N=`N', rho=`rho'"
display as text "========================================================================="
matrix list Results, format(%9.6f)

// Export results to text file for easier reading
local txt_filename "`output_dir'/exp5_model`model'_N`N'_rho`rho'.txt"
file open txtfile using "`txt_filename'", write replace
file write txtfile "=========================================================================" _n
file write txtfile "EXPERIMENT 5: NON-STATIONARY TIME-VARYING ERRORS - TABLE 2 REPLICATION" _n
file write txtfile "AL SADOON ET AL. (2019) - ENDOGENOUS SELECTION RESULTS" _n
file write txtfile "=========================================================================" _n
file write txtfile "Experiment: Non-stationary time-varying error components" _n
file write txtfile "- Time-varying variance: εᵢₜ or uᵢₜ multiplied by Bernoulli(1,2)" _n
file write txtfile "- Time-varying correlation: ϑ multiplied by {0.5, 1, 2}" _n
...
file write txtfile "Selection Type: Endogenous with time-varying parameters" _n
...
local filename "exp5_model`model'_N`N'_rho`rho'.dta"
preserve
clear
...
svmat Results, names(col)
gen str model = "`model'"
gen str experiment_type = "exp5_nonstationary_errors"
gen str date_time = "`c(current_date)' `c(current_time)'"
local csv_filename "`output_dir'/exp5_model`model'_N`N'_rho`rho'.csv"
...
display as text ""
display as text "========================================================================="
display as text "EXPERIMENT 5 COMPLETE FOR N=`N', MODEL=`model', RHO=`rho'"
display as text "========================================================================="
