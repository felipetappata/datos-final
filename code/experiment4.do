/* ========================================================================= 
   EXPERIMENT 4: REDUCING CORRELATION OF ERRORS - TABLE 2 REPLICATION
   =========================================================================
   
   This script implements Experiment IV from Table 2 of Al Sadoon et al. (2019):
   "Reducing the correlation of the errors: ρ=0.25"
   
   This experiment tests the sensitivity of AB and System GMM estimators when
   the correlation between the error components is reduced. The correlation
   parameters are changed from 0.5 to 0.25, yielding:
   εᵢₜ = εᵢₜ⁰ + 0.25 * uᵢₜ  and  αᵢ = αᵢ⁰ + 0.25 * ηᵢ
   
   This implies a correlation coefficient of 0.2425 = 0.25/√(1+0.25²).
   
   Command line usage:
   stata -b do experiment4.do N model rho
...
if "`1'" == "" | "`2'" == "" | "`3'" == "" {
    display as error "Usage: stata -b do experiment4.do N model rho [output_dir]"
...
// ========================================================================
// SIMULATION PARAMETERS - EXPERIMENT 4: REDUCING CORRELATION OF ERRORS
// ========================================================================
...
// EXPERIMENT 4 MODIFICATION: Reduced correlation parameters
local sigma_alpha0 1         // Standard deviation of alpha_i^0 ~ N(0,1)
local sigma_eps0 1           // Standard deviation of epsilon_it^0 ~ N(0,1)
local theta_param 0.25       // CHANGED: Correlation parameter alpha_i = alpha_i^0 + theta*eta_i
local vartheta_param 0.25    // CHANGED: Correlation parameter eps_it = eps_it^0 + vartheta*u_it
...
display as text ""
display as text "========================================================================="
display as text "EXPERIMENT 4: REDUCING CORRELATION OF ERRORS - ENDOGENOUS SELECTION"
display as text "========================================================================="
display as text "Parameters: N=`N', Model=`model', rho=`rho', Replications=`reps'"
display as text "EXPERIMENT: Reducing correlation of errors (correlation parameter = 0.25)"
display as text "Selection: 15% attrition, ENDOGENOUS (theta=`theta_param', vartheta=`vartheta_param')"
display as text "Implied correlation: " %5.3f `implied_corr' " (vs baseline 0.447)"
display as text "========================================================================="
display as text ""
...
forvalues rep = 1/`reps' {
    ...
    // ====================================================================
    // DATA GENERATION PROCESS (Modified correlation structure)
    // ====================================================================
    ...
    quietly {
        ...
        // Reduced endogenous selection: lower correlation between equations
...
display as text ""
display as text "========================================================================="
display as text "EXPERIMENT 4 SIMULATION COMPLETED"
display as text "========================================================================="
...
matrix Results[1,9] = 4

// Display final results table
display as text ""
display as text "========================================================================="
display as text "EXPERIMENT 4 FINAL RESULTS - REDUCING CORRELATION OF ERRORS"
display as text "Model `model', N=`N', rho=`rho'"
display as text "========================================================================="
matrix list Results, format(%9.6f)

// Export results to text file for easier reading
local txt_filename "`output_dir'/exp4_model`model'_N`N'_rho`rho'.txt"
file open txtfile using "`txt_filename'", write replace
file write txtfile "=========================================================================" _n
file write txtfile "EXPERIMENT 4: REDUCING CORRELATION OF ERRORS - TABLE 2 REPLICATION" _n
file write txtfile "AL SADOON ET AL. (2019) - ENDOGENOUS SELECTION RESULTS" _n
file write txtfile "=========================================================================" _n
file write txtfile "Experiment: Reducing correlation of errors (correlation parameter = 0.25)" _n
...
file write txtfile "Selection Type: Endogenous (theta=0.25, vartheta=0.25)" _n
file write txtfile "Implied Correlation: " %5.3f (`implied_corr') " (vs baseline 0.447)" _n
...
local filename "exp4_model`model'_N`N'_rho`rho'.dta"
preserve
clear
...
svmat Results, names(col)
gen str model = "`model'"
gen str experiment_type = "exp4_reduced_correlation"
gen str date_time = "`c(current_date)' `c(current_time)'"
local csv_filename "`output_dir'/exp4_model`model'_N`N'_rho`rho'.csv"
...
display as text ""
display as text "========================================================================="
display as text "EXPERIMENT 4 COMPLETE FOR N=`N', MODEL=`model', RHO=`rho'"
display as text "========================================================================="
